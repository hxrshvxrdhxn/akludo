...
const ListView = React.createClass({
	...
	componentWillMount() {
		...
		// set filters
		if(this.props.params.columns) this.props.dispatch(setActiveColumns(this.props.params.columns));
		if(this.props.params.filters) this.props.dispatch(setActiveFilters(this.props.params.filters));
		if(this.props.params.sort) this.props.dispatch(setActiveSort(this.props.params.sort));
		if(this.props.params.search) this.props.dispatch(setActiveSearch(this.props.params.search));
		if(this.props.params.page) this.props.dispatch(setCurrentPage(this.props.params.page));
	...
	componentWillReceiveProps(nextProps) {
		...
		if(this.props.params.columns && this.props.params.columns !== nextProps.params.columns) {
			console.log('Changing columns');
			changed = true;
			this.props.dispatch(setActiveColumns(nextProps.params.columns));
		}
		if(this.props.params.filters && JSON.stringify(this.props.params.filters) !== JSON.stringify(nextProps.params.filters)) {
			console.log('Changing filters');
			changed = true;
			this.props.dispatch(setActiveFilters(nextProps.params.filters));
		}
		if(this.props.params.sort && this.props.params.sort !== nextProps.params.sort) {
			console.log('Changing sort');
			changed = true;
			this.props.dispatch(setActiveSort(nextProps.params.sort));
		}
		if(this.props.params.search && this.props.params.search !== nextProps.params.search) {
			console.log('Changing search');
			changed = true;
			this.props.dispatch(setActiveSearch(nextProps.params.search));
		}
		if(this.props.params.page && this.props.params.page !== nextProps.params.page) {
			console.log('Changing page');
			changed = true;
			this.props.dispatch(setCurrentPage(nextProps.params.page));
		}
		if(changed){
			this.props.dispatch(loadInitialItems());
		}
		// if (JSON.stringify({o: nextProps.location.query}) !== JSON.stringify({o: this.props.location.query})) {
		// 	this.parseQueryParams(nextProps.location.query);
		// 	this.props.dispatch(loadInitialItems());
		// }
	},
	/**
	 ...
	 */
	parseQueryParams(nextProps) {
		// todo pull from props passed
		const query = nextProps || this.props.location.query;
	...
	renderHeader() {
		const items = this.props.items;
		let {autocreate, nocreate, plural, singular} = this.props.currentList;
		if(this.props.params.nocreate) nocreate = true;
	...
	renderBlankState() {
		...
		return (
			<Container>
				...
				<BlankState heading={`No ${this.props.currentList.plural.toLowerCase()} found...`}
							style={{marginTop: 40}}>
					{button}
				</BlankState>
			</Container>
					...
					{(this.props.loading) ? (
						...
						<div>
							<ItemsTable
								...
								dispatch={this.props.dispatch}
								rowBtns={this.props.currentList.rowButtons}
								rowButtonsDynamic={this.props.currentList.rowButtonsDynamic}
							/>
