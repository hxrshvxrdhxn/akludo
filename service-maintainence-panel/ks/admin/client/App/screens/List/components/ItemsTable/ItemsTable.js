...
const ItemsTable = React.createClass({
	...
	renderCols () {
		...
		if(this.props.list.rowButtonsDynamic && this.props.list.rowButtonsDynamic.length) {
			this.props.list.rowButtonsDynamic.forEach(btn => {
				if(!btn.itemPage && eval(btn.visible)()) cols.push(<col width={btn.width} key={btn.id} />);
			});
		}
	...
	renderHeaders () {
		...
		if(this.props.list.rowButtonsDynamic && this.props.list.rowButtonsDynamic.length){
			this.props.list.rowButtonsDynamic.forEach(btn => {
				if(!btn.itemPage && eval(btn.visible)()) rMap.push(<th>&nbsp;</th>);
			});
		}
		if(this.props.list.rowButtons && this.props.list.rowButtons.length){
			this.props.list.rowButtons.forEach(btn => {
				if(!btn.itemPage && eval(btn.visible)()) rMap.push(<th><i className={btn.icon}></i></th>);
			});
		}
				...
				{items.results.map((item, i) => {
					return (
						<TableRow key={item.id}
							...
							id={item.id}
							item={item}
							rowButtons={this.props.list.rowButtons}
							rowButtonsDynamic={this.props.list.rowButtonsDynamic}
							{...this.props}
						/>
					);
				})}
