...
const ItemsRow = React.createClass({
	...
	renderRow (item) {
		...
		var cells = this.props.columns.map((col, i) => {
			...
			if(col.field && col.field.link && col.field.link.search(/^\(function/) > -1) {
				let fn = eval(col.field.link);
				userLink = fn.call(this, item);
			} else if (col.field && col.field.link) userLink = col.field.link;
			var linkTo = !i ? `${Keystone.adminPath}/${this.props.list.path}/${itemId}` : userLink ? userLink : undefined;
			// console.log(col.field && col.field.link, linkTo);
			return <ColumnType key={col.path} list={this.props.list} col={col} data={item} linkTo={linkTo} />;
		});

		// rowButtonsDynamic
		if(this.props.rowButtonsDynamic && this.props.rowButtonsDynamic.length){
			this.props.rowButtonsDynamic.forEach(btn => {
				if(!btn.itemPage && eval(btn.visible)(item)) {
					let nm = eval(btn.val)(item);
					console.log('....................', nm, '>>>>>>>>>>>>>>>>>>', btn, item);
					if(nm === '_button_') {
						if(btn.linkField){
							cells.push(<a href={item.fields[btn.linkField]} target="_blank"><td><button className='row-button' style={{color: btn.color}} onClick={(e) => window[btn.fn](item, e, () => this.props.dispatch(loadItems()))}><i className={btn.icon}></i> {btn.name}</button></td></a>)
						} else {
							cells.push(<td><button className='row-button' style={{color: btn.color}} onClick={(e) => window[btn.fn](item, e, () => this.props.dispatch(loadItems()))}><i className={btn.icon}></i> {btn.name}</button></td>)

						}
					} else cells.push(<td>{nm}</td>);
				}
			});
		}
		// rowButtons
		if(this.props.rowButtons && this.props.rowButtons.length){
			this.props.rowButtons.forEach(btn => {
				if(!btn.itemPage && eval(btn.visible)(item)) cells.push(<td><button className='row-button' style={{color: btn.color}} onClick={(e) => window[btn.fn](item, e, () => this.props.dispatch(loadItems()))}><i className={btn.icon}></i> {btn.name}</button></td>);
			});
		}
