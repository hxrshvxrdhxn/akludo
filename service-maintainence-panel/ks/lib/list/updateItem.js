...
const handleInputs = require('./safety');
...
function updateItem (item, data, options, callback) {
	...
	function doFieldUpdate (field, done) {
		...
		data = handleInputs(field, data, updateErrors, addFieldUpdateError);
		...
		function (doneUpdate) {
			async.each(fields, doFieldUpdate, function () {
				if (Object.keys(updateErrors).length) {
					let keys = Object.keys(updateErrors);
					let detail = 'field errors';
					if (keys.length)
							{ detail = updateErrors[keys[0]].detail.message; }
					return doneUpdate({
						error: detail,
						detail: updateErrors,
					});
